...
public class Hero extends Warrior {
    ...
    public static String[] heroName = { "能量条", "能量条", "能量条", "能量条","能量条" };
    public static int[] initHeroChongTime = { 60, 90, 85, 95 };
    public static int[] initHeroInvincibleTime = { 60, 85, 95, 100 };


    public static boolean[] isGot={true,false,false,false,false};
    public static int[] initHeroXiangYun={80,90,85,80};
    ...
    public static int[] heroChongTime={0,0,0,0,0};
    public static int[] heroInvincibleTime={0,0,0,0,0};
    public static int[] heroXiangYun={0,0,0,0,0};
    public static int[] heroSuckStar={0,0,0,0,0};

    public static int[] heroChongTimeLV={0,0,0,0,0};
    public static int[] heroInvincibleTimeLV={0,0,0,0,0};
    public static int[] heroXiangYunLV={0,0,0,0,0};
    public static int[] heroSuckStarLV={0,0,0,0,0};
	...
	private float G_Y = 1.9f;// 重力Y方向加速度
    ...
    public Hero(int x, int y, int type) {
        ...
        if (GameVariables.initPet[0][0] == 1) {
            pet2 = new Pet(GameVariables.initPet[0][1] + GameVariables.isLandPetGot.length, x + 130, y - 180);
            G_Y -= 0.4;
        }
        if (GameVariables.initPet[1][0] == 1) {
            pet = new Pet(GameVariables.initPet[1][1], x + 200, y);
            G_Y -= 0.4;
        }
        switch (type) {
            case 0:
                bodyWidth = 66;
                bodyHeight = 70;
                motion = new Motion("/bee/bee.anu", posX, posY);
                break;
            case 1:
                bodyWidth = 66;
                bodyHeight = 70;
                motion = new Motion("/cache/cache.anu", posX, posY);
                break;
            case 2:
                bodyWidth = 66;
                bodyHeight = 70;
                motion = new Motion("/paoche/paoche.anu", posX, posY);
                break;
            case 3:
                bodyWidth = 66;
                bodyHeight = 70;
                motion = new Motion("/qin/qin.anu", posX, posY);
                break;
        }

//        deadMotion = new Motion("/game/car" + (type+1) + "/car" + (type+1) + ".anu", posX, posY);
        chongfengMotion = new Motion("/effect/chongfeng.anu", posX, posY);
        deadMotion = chongfengMotion;
    }
...
//                        safeTime();
                ...
                case STAGE_CHONG:
                    isJump = false;
                    motionState = 1;   System.out.println("STAGE_CHONG------");
                    maxChongTime = heroChongTime[GameVariables.heroIndex]/10 + updateChongTime;
...
//				tempX -= speed;
                ...
                case STAGE_INVINCIBLE:
                    ...
                    if (null == imgHuDun) {
                        imgHuDun = Globe.getImage("effect/hudun.png");
                    }
                    isInvincible = true;
                    setState(STAGE_MOVE);
                    InvincibleBuffTime = System.currentTimeMillis();
                    maxInvincibleTime += heroInvincibleTime[GameVariables.heroIndex]/10 + updateInvincibleTime;
                    break;
                case STAGE_XIJIN:
                    isXiJin = true;
            ...
            if (isChong) {
                ...
                chongfengMotion.update(tempX-50, tempY);
            } else {
...
//        System.out.println("chongBuffTime:"+chongBuffTime);
//        System.out.println("------------------------");
//        System.out.println("************************");

        chongTime = (int) (System.currentTimeMillis() - chongBuffTime) / 1000;

//        System.out.println("maxChongTime:"+maxChongTime);
//        System.out.println("updateChongTime:"+updateChongTime);
//        System.out.println("chongTime:" + chongTime);
//        System.out.println("------------------------");
//        System.out.println("************************");
